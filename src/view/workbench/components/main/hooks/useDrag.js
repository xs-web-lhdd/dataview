...
// * 拖拽到编辑区域里
export const dragHandle = async (e) => {
  e.preventDefault()
  ...
  try {
    ...
    const dropData = JSONParse(drayDataString)
...
// * 框选
export const mousedownBoxSelect = (e, item) => {
  if (e.which == 2) return
  ...
  const mousemove = throttle((moveEvent) => {
    ...
    chartEditStore.setEditCanvas('isSelect', true)
  ...
  const mouseup = () => {
    ...
    mousemove.cancel()
    chartEditStore.setEditCanvas('isSelect', false)
